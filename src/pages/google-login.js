...
export default function GoogleLogin() {
  ...
  const { user, loginWithGoogle, error, logout, onAuthStateChanged } =
    useAuth();
  console.log(user, "user in google login page");
  ...
  return (
    <>
          ...
          <div className="wrapper-left">
            <div className="part-one">
              <h1 style={{ fontSize: "12px" }}>
                {" "}
                YOU NEED TO GET THE LOGGED IN USERS DATA AND THEN SAVE THE DATA
                SO THAT AFTER REFRESH IT STAYS{" "}
              </h1>
              <h3>Get started</h3>
            ...
            <div className="buttons">
              <button onClick={logout} className="btn-one">
                Create an account{" "}
      ...
      <div>
        ...
        <h1 style={{ color: "#000" }}>{user?.email}</h1>
        <h1 style={{ color: "#000" }}>{user?.displayName}</h1>
      </div>
    </>
  );
}
